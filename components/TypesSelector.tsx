...
const TypesSelector = ({ setFilters, filters }: Props) => {
  ...
  return (
    <div className="flex overflow-auto flex-shrink-0 pb-3 whitespace-nowrap">
        {types.map((type, index) => {
          return (
            <div
              key={index}
              className={`border-primary-color border px-2 w-full mx-1 rounded-md hover:cursor-pointer ${
                filters.includes(type) && "bg-primary-color text-white"
              }`}
              onClick={() => {
